    ...
    def __call__(self, x, train=True):
        ...
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE) as scope:
            ...
            self.beta = tf.get_variable(
                "beta", [shape[-1]], initializer=tf.constant_initializer(0.0)
            )
...
def binary_cross_entropy_with_logits(logits, targets, name=None):
    ...
    with ops.op_scope([logits, targets], name, "bce_loss") as name:
        ...
        return tf.reduce_mean(
            -(
                logits * tf.log(targets + eps)
                + (1.0 - logits) * tf.log(1.0 - targets + eps)
            )
        )
...
def deconv2d(
    ...
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        w = tf.get_variable(
            "w",
            [k_h, k_h, output_shape[-1], input_.get_shape()[-1]],
