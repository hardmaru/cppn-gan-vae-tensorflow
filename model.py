...
class CPPNVAE:
    def __init__(
        ...
        self.balanced_loss = (
            1.0 * self.g_loss + 1.0 * self.vae_loss
        ...
        with tf.variable_scope(self.model_name + "_opt", reuse=tf.AUTO_REUSE) as scope:
            ...
            self.g_opt = tf.train.AdamOptimizer(
                self.learning_rate, beta1=self.beta1
    ...
    def create_vae_loss_terms(self):
        ...
        reconstr_loss = -tf.reduce_sum(
            self.batch_flatten * tf.log(1e-10 + self.batch_reconstruct_flatten)
            + (1 - self.batch_flatten)
            * tf.log(1e-10 + 1 - self.batch_reconstruct_flatten),
            1,
        )
    ...
    def create_gan_loss_terms(self):
        ...
        self.d_loss_real = binary_cross_entropy_with_logits(
            tf.ones_like(self.D_right), self.D_right
        )
        self.d_loss_fake = binary_cross_entropy_with_logits(
            tf.zeros_like(self.D_wrong), self.D_wrong
        )
    ...
    def discriminator(self, image, reuse=False):
        ...
        h3 = linear(
            tf.reshape(h2, [self.batch_size, -1]), 1, self.model_name + "_d_h2_lin"
        )
    ...
    def generator(self, gen_x_dim=26, gen_y_dim=26, reuse=False):
        ...
        for i in range(1, self.net_depth_g):
            H = tf.nn.tanh(
                fully_connected(H, n_network, self.model_name + "_g_tanh_" + str(i))
            )
